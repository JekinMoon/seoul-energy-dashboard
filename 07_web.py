...
# 4. 상단 헤더 (모바일 flex-wrap 대응)
placeholder = st.empty()
now = datetime.datetime.now()
now_str = now.strftime('%Y-%m-%d %H:%M')

placeholder.markdown(f"""
<div style="background:#1B3A6B; border-radius:12px; padding:20px 24px; margin-bottom:25px; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
    <div class="header-inner" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:8px;">
        <div>
            <div style="font-size:1.3rem; font-weight:800; color:white;">🔌 서울시 전력 수요 관제 시스템</div>
            <div style="font-size:0.85rem; opacity:0.7; color:white; margin-top:4px; font-family:'DM Mono',monospace;">Seoul Energy Demand Simulation Dashboard</div>
        </div>
        <div style="color:white; font-family:'DM Mono',monospace; font-size:0.85rem; opacity:0.8;">🕐 {now_str}</div>
    </div>
</div>
""", unsafe_allow_html=True)
