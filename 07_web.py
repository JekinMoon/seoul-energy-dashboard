...
@st.cache_data
def load_data():
    path = "df_final_v2.csv" 
    try:
        df = pd.read_csv(path)
        df['datetime'] = pd.to_datetime(df['datetime'])
        return df
    except:
        path = r"C:\Users\82104\Desktop\Project2\seoul_power_demand_forecast\1_data\processed\df_final_v2.csv"
        df = pd.read_csv(path)
        df['datetime'] = pd.to_datetime(df['datetime'])
        return df
...
menu = st.sidebar.radio(
    label="Select View", # 라벨은 넣되 label_visibility로 숨김 처리 가능
    options=["⚡ 전력 수요 시뮬레이터", "🔍 상세 분석 리포트"],
    label_visibility="collapsed" # "이동할 페이지..." 문구를 완전히 숨김
)
# 공통 시뮬레이션 설정 (사이드바)
st.sidebar.markdown("---")
...
if menu == "⚡ 전력 수요 시뮬레이터":
    ...
    with col_right:
        ...
        st.info(f"과거 유사 사례 분석 결과, 해당 기상 조건에서는 **오후 {peak_hour}시**를 기점으로 전력 수요가 급증하는 경향을 보입니다.")
...
# --- 페이지 2: 상세 분석 리포트 ---
    ...
    with b_right:
        ...
        st.success("""
        **💡 분석 결과 요약** \n
        본 모델은 서울시 전력 수요 변동의 **97% 이상**을 정확히 설명하며,  
        실제 수요와의 오차가 **평균 3% 미만**인 높은 신뢰도 분석 결과를 제공합니다.
        """)
