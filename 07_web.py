...
<div style="background:#1B3A6B; border-radius:12px; padding:20px 24px; margin-bottom:25px; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
    <div class="header-inner" style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:8px;">
        ...
        <div id="live-clock" style="color:white; font-family:'DM Mono',monospace; font-size:0.85rem; opacity:0.8;">🕐 --:--</div>
    </div>
</div>
<script>
    function updateClock() {{
        ...
        if (el) el.innerText = '🕐 ' + y + '-' + mo + '-' + d + ' ' + h + ':' + mi;
    }}
    updateClock();
...
if page == "⚡ 전력 수요 시뮬레이션":
    ...
    with st.container():
        ...
        with c4:
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-card-label">🌿 탄소 배출 추정 (tCO₂)</div>
                <div class="kpi-card-value" style="color:#111111;">{carbon_ton:,.0f}</div>
