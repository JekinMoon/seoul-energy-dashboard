...
# ==========================================
# 4. 사이드바 (양방향 동기화 적용 버전)
# ==========================================

# --- [추가] 세션 상태 초기화: 값이 없으면 기본값으로 생성 ---
...
# --- [추가] 콜백 함수: 한쪽이 변하면 다른 쪽도 동기화 ---
def update_val(key_from, key_to):
...
with c1_temp:
    st.slider("t_slider", -10.0, 40.0, key="t_slider_key", 
              value=st.session_state.s_temp, 
              on_change=update_val, args=("t_slider_key", "s_temp"), 
              label_visibility="collapsed")
with c2_temp:
    st.number_input("t_input", -10.0, 40.0, key="s_temp", 
                    on_change=update_val, args=("s_temp", "t_slider_key"),
...
with c1_hum:
    st.slider("h_slider", 0, 100, key="h_slider_key", 
              value=st.session_state.s_hum,
              on_change=update_val, args=("h_slider_key", "s_hum"), 
              label_visibility="collapsed")
with c2_hum:
    st.number_input("h_input", 0, 100, key="s_hum", 
                    on_change=update_val, args=("s_hum", "h_slider_key"),
...
with c1_wind:
    st.slider("w_slider", 0.0, 15.0, key="w_slider_key", 
              value=st.session_state.s_wind,
              on_change=update_val, args=("w_slider_key", "s_wind"), 
              label_visibility="collapsed")
with c2_wind:
    st.number_input("w_input", 0.0, 15.0, key="s_wind", 
                    on_change=update_val, args=("s_wind", "w_slider_key"),
                    step=0.5, label_visibility="collapsed")
...
with c1_hour:
    st.slider("hr_slider", 0, 23, key="hr_slider_key", 
              value=st.session_state.s_hour,
              on_change=update_val, args=("hr_slider_key", "s_hour"), 
              label_visibility="collapsed")
with c2_hour:
    st.number_input("hr_input", 0, 23, key="s_hour", 
                    on_change=update_val, args=("s_hour", "hr_slider_key"),
                    step=1, label_visibility="collapsed")

# 최종 변수 할당 (예측 로직에서 사용됨)
