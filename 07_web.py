...
# 2. CSS 설정 (상단 여백 3rem 확보, 슬라이더 기본 색상 유지)
st.markdown("""
<style>
    ...
    /* 상단 꺽쇠(토글) 아래에 대시보드가 예쁘게 안착하도록 여백 확보 */
    .block-container {
...
# 3. 사이드바 설정 및 양방향 슬라이더 연동 로직 (Session State 활용)
...
# 4. 상단 헤더 (불필요한 탭 문구를 제거하고 심플하게 타이틀만 유지)
st.markdown(f"""
<div style="background: #1B3A6B; border-radius: 12px; padding: 25px 32px; margin-bottom: 25px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
            <div style="font-size: 1.5rem; font-weight: 800; color: white;">🔌 서울시 전력 수요 관제 시스템</div>
            <div style="font-size: 0.9rem; opacity: 0.7; color: white; margin-top: 4px; font-family: 'DM Mono', monospace;">Seoul Energy Demand Simulation Dashboard</div>
        </div>
        <div style="color: white; font-family: 'DM Mono', monospace; font-size: 0.9rem; opacity: 0.8;">{now_str}</div>
    </div>
</div>
""", unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with c4:
            ...
            <div class="kpi-card">
                ...
                <div class="kpi-card-value" style="font-size:1.9rem; white-space:nowrap; overflow:hidden; color:#111111;">{carbon_ton:,.0f} <span style="font-size:0.85rem; color:#111111; vertical-align:middle;">tCO₂</span></div>
