...
# 0. 상수 정의 (매직 넘버 제거)
# ==========================================
CARBON_FACTOR_SOLAR   = 0.3500   # 태양광 주도 시간대 (11~15시)
CARBON_FACTOR_PEAK    = 0.5200   # 화력 기동 시간대 (16~20시)
CARBON_FACTOR_DEFAULT = 0.4594   # 평균 배출 계수

CARBON_BADGE_HIGH     = 2800     # tCO₂ 기준: 고탄소 경보 (계수별 보정)
CARBON_BADGE_MID      = 2300     # tCO₂ 기준: 중간 경보

PEAK_HOUR_START       = 14
PEAK_HOUR_END         = 17

ALERT_DELTA_EMERGENCY = 15       # 평균 대비 +15% 이상 → 비상
ALERT_DELTA_WARNING   = 5        # 평균 대비 +5% 이상  → 경계
ALERT_TEMP_EMERGENCY  = 33       # 폭염 기준
ALERT_TEMP_WARNING    = 30
ALERT_TEMP_COLD_WARN  = 0
ALERT_TEMP_COLD_EMERG = -5
...
<style>
    ...
    /* Setpoint 선택 버튼 활성화 강조 */
    .sp-btn-active {
        background: #1B3A6B !important; color: white !important;
        border: 2px solid #1B3A6B !important; border-radius: 6px;
    }
...
def calc_scenario_stats(df):
    ...
    peak_avg    = df[df['hour'].between(PEAK_HOUR_START, PEAK_HOUR_END)]['전력사용량(MWh)'].mean()
...
def get_dynamic_carbon_factor(hour):
    """덕커브(Duck Curve) 반영: 시간대별 동적 탄소 배출 계수"""
    if 11 <= hour <= 15:
        return CARBON_FACTOR_SOLAR, "🟢 저탄소 (태양광 주도)"
    elif 16 <= hour <= 20:
        return CARBON_FACTOR_PEAK, "🔴 고탄소 (화력 기동)"
    else:
        return CARBON_FACTOR_DEFAULT, "🟡 일반 (평균 배출)"

def get_series_value(series_row, key, default=None):
    """pandas Series에서 안전하게 값을 가져오는 헬퍼 함수 (Series.get 대체)"""
    if key in series_row.index:
        return series_row[key]
    return default
...
# ── 슬라이더-숫자입력 완전 동기화 ──────────────────────────────────
# 핵심 원칙:
#   - 슬라이더의 key(예: "s_temp_sl")가 유일한 진짜 값 저장소
#   - number_input on_change 시 슬라이더 key를 직접 덮어써서 동기화
#   - 슬라이더는 session_state에 자동 반영되므로 별도 on_change 불필요
#   - number_input의 value는 매 렌더링마다 슬라이더 key에서 읽어 항상 일치

# 초기값 세팅 (최초 1회만)
if 's_temp_sl' not in st.session_state: st.session_state['s_temp_sl'] = 25.0
if 's_hum_sl'  not in st.session_state: st.session_state['s_hum_sl']  = 50
if 's_wind_sl' not in st.session_state: st.session_state['s_wind_sl'] = 2.0
if 's_hour_sl' not in st.session_state: st.session_state['s_hour_sl'] = 14

# 기온
st.sidebar.write("예상 기온 (°C)")
c1_temp, c2_temp = st.sidebar.columns([7, 3])
with c1_temp:
    st.slider("", -10.0, 40.0, step=0.5, key="s_temp_sl",
              label_visibility="collapsed")
with c2_temp:
    st.number_input("", -10.0, 40.0, step=0.5, key="s_temp_ni",
                    value=float(st.session_state['s_temp_sl']),
                    on_change=lambda: st.session_state.update({'s_temp_sl': float(st.session_state['s_temp_ni'])}),
                    label_visibility="collapsed")
s_temp = float(st.session_state['s_temp_sl'])

# 습도
st.sidebar.write("예상 습도 (%)")
c1_hum, c2_hum = st.sidebar.columns([7, 3])
with c1_hum:
    st.slider("", 0, 100, step=5, key="s_hum_sl",
              label_visibility="collapsed")
with c2_hum:
    st.number_input("", 0, 100, step=5, key="s_hum_ni",
                    value=int(st.session_state['s_hum_sl']),
                    on_change=lambda: st.session_state.update({'s_hum_sl': int(st.session_state['s_hum_ni'])}),
                    label_visibility="collapsed")
s_hum = int(st.session_state['s_hum_sl'])

# 풍속
st.sidebar.write("예상 풍속 (m/s)")
c1_wind, c2_wind = st.sidebar.columns([7, 3])
with c1_wind:
    st.slider("", 0.0, 15.0, step=0.5, key="s_wind_sl",
              label_visibility="collapsed")
with c2_wind:
    st.number_input("", 0.0, 15.0, step=0.5, key="s_wind_ni",
                    value=float(st.session_state['s_wind_sl']),
                    on_change=lambda: st.session_state.update({'s_wind_sl': float(st.session_state['s_wind_ni'])}),
                    label_visibility="collapsed")
s_wind = float(st.session_state['s_wind_sl'])

# 시간
st.sidebar.write("예상 시간 (시)")
c1_hour, c2_hour = st.sidebar.columns([7, 3])
with c1_hour:
    st.slider("", 0, 23, step=1, key="s_hour_sl",
              label_visibility="collapsed")
with c2_hour:
    st.number_input("", 0, 23, step=1, key="s_hour_ni",
                    value=int(st.session_state['s_hour_sl']),
                    on_change=lambda: st.session_state.update({'s_hour_sl': int(st.session_state['s_hour_ni'])}),
                    label_visibility="collapsed")
s_hour = int(st.session_state['s_hour_sl'])
...
if not df.empty:
    ...
    input_row['dayofweek']    = 2   # 베이스라인: 평일(수요일) 고정
    input_row['is_holiday']   = 0
    input_row['is_peak_hour'] = 1 if PEAK_HOUR_START <= s_hour <= PEAK_HOUR_END else 0
    input_row['기온(°C)']     = s_temp
    input_row['습도(%)']      = s_hum
    input_row['강수량(mm)']   = 0.0
    input_row['CDD']          = max(0, s_temp - 24)
    input_row['HDD']          = max(0, 18 - s_temp)
    input_row['is_heatwave']  = 1 if s_temp >= ALERT_TEMP_EMERGENCY else 0
    input_row['temp_3h_mean'] = s_temp

    # ── 수정: pandas Series에서 안전하게 값 읽기 (Series.get() 오류 방지) ──
    temp_6h_max_val = get_series_value(input_row, 'temp_6h_max', default=s_temp)
    input_row['temp_6h_max'] = max(s_temp, temp_6h_max_val)
...
# ── 수정: 탄소 배지 임계값을 계수 비율로 보정 ──────────────────────
carbon_high_threshold = CARBON_BADGE_HIGH * (current_carbon_factor / CARBON_FACTOR_DEFAULT)
carbon_mid_threshold  = CARBON_BADGE_MID  * (current_carbon_factor / CARBON_FACTOR_DEFAULT)

# ── 수정: YoY 계산 — 샘플 수 최소 기준 강화 + ±1시간 범위 확대 ──
yoy_delta = None
same_cond = df[
    (df['hour'].between(max(0, s_hour - 1), min(23, s_hour + 1))) &
    (df['month'] == s_month)
]
MIN_YOY_SAMPLES = 20
if len(same_cond) >= MIN_YOY_SAMPLES * 2:
    a23 = same_cond[same_cond['datetime'].dt.year == 2023]['전력사용량(MWh)'].mean()
    a24 = same_cond[same_cond['datetime'].dt.year == 2024]['전력사용량(MWh)'].mean()
    if a23 > 0 and not np.isnan(a23) and not np.isnan(a24):
        yoy_delta = ((a24 - a23) / a23) * 100

# 경보 로직
if delta_pct >= ALERT_DELTA_EMERGENCY or s_temp >= ALERT_TEMP_EMERGENCY or s_temp <= ALERT_TEMP_COLD_EMERG:
    alert_status, alert_color, alert_emoji = "비상", "#C0392B", "🔴"
elif delta_pct >= ALERT_DELTA_WARNING or s_temp >= ALERT_TEMP_WARNING or s_temp <= ALERT_TEMP_COLD_WARN:
    alert_status, alert_color, alert_emoji = "경계", "#E67E22", "🟡"
else:
    alert_status, alert_color, alert_emoji = "정상", "#27AE60", "🟢"

is_peak = PEAK_HOUR_START <= s_hour <= PEAK_HOUR_END

# ==========================================
# 6. 상단 헤더 (Python 측 시계로 안정적 표시)
# ==========================================
...
<div style="background:#1B3A6B;border-radius:12px;padding:20px 24px;margin-bottom:0px;
  ...
  <div style="color:rgba(255,255,255,0.8);font-family:'DM Mono',monospace;font-size:0.85rem;">
    🕐 {now_str}
  </div>
</div>
""", unsafe_allow_html=True)
...
if page == "⚡ 전력 수요 시뮬레이션":
    ...
    carbon_badge = "badge-red" if carbon_ton > carbon_high_threshold else ("badge-orange" if carbon_ton > carbon_mid_threshold else "badge-green")
    carbon_msg   = carbon_status_msg
    yoy_badge    = "badge-red" if (yoy_delta or 0) > 0 else "badge-green"
    yoy_val      = f"{yoy_delta:+.1f}%" if yoy_delta is not None else "-"
    yoy_msg      = f"{'⬆' if (yoy_delta or 0)>0 else '⬇'} 전년 동기 대비" if yoy_delta is not None else "데이터 없음"
    ...
    with col1:
        with st.container(border=True):
            ...
            colors   = ['#C0392B' if PEAK_HOUR_START <= h <= PEAK_HOUR_END else '#1B3A6B' for h in hour_avg['hour']]
...
# ==========================================
    ...
    with c2:
        with st.container(border=True):
            ...
            # ── 수정: 선택된 버튼을 시각적으로 강조 표시 ──────────────
            sp1, sp2, sp3 = st.columns(3)
            btn_options = [1, 2, 3]
            for col, val in zip([sp1, sp2, sp3], btn_options):
                with col:
                    is_selected = (_setpoint_adj == val)
                    btn_label   = f"{'✅ ' if is_selected else ''}{btn_sign}{val}°C"
                    btn_type    = "primary" if is_selected else "secondary"
                    if st.button(btn_label, use_container_width=True, type=btn_type, key=f"sp_btn_{val}"):
                        st.session_state["setpoint_adj"] = 0 if is_selected else val
                        st.rerun()
    ...
    actions_html = "<br>".join([f"&nbsp;&nbsp;&nbsp;&nbsp;{i+1}. {text}" for i, text in enumerate(action_texts)]) \
                   if action_texts else "&nbsp;&nbsp;&nbsp;&nbsp;- 시스템 관망 (추가 제어 불필요)"
