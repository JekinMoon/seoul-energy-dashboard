...
if menu == "⚡ 전력 수요 시뮬레이션":
    ...
    # 2️⃣ 상단 핵심 지표 (KPI Columns)
    kpi1, kpi2, kpi3, kpi4 = st.columns([3, 2, 2, 3])
    with kpi1:
        st.metric("⚡ 총 예측 수요", f"{total_load:,.0f} MWh")
    with kpi2:
        st.metric("🌡️ 설정 기온", f"{s_temp} °C")
    with kpi3:
        st.metric("💧 설정 습도", f"{s_hum} %")
    with kpi4:
        st.write("**관제 상태**")
        st.markdown(f":{color}[**{status}**]")
    ...
    with desc_col:
        st.success(f"""
        - **🏠 기본 전력 수요**: 기상 무관, 24시간 일정 소비 전력 (기저 부하)
